/**
* Author: Indy256
* Description: Let there be a tree G of n vertices, with an arbitrary root.
* The essence of this tree decomposition is to split the tree into several paths
* so that we can reach the root vertex from any v by traversing at most  
* \log n  paths. In addition, none of these paths should intersect with another.
* It is clear that if we find such a decomposition for any tree, it will allow 
* us to reduce certain single queries of the form “calculate something on the
* path from a to b” to several queries of the type ”calculate something on the 
* segment [l, r] of the Kth path”.
*/
